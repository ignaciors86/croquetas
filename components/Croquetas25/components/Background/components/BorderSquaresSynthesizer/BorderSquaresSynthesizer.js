...
const BorderSquaresSynthesizer = ({
  ...
  useEffect(() => {
    ...
    const handleSynthesizerEvent = (type, data = {}) => {
      ...
      if (type === 'beat' && !data.shouldBeSolid) {
        ...
        if (intensity < MIN_INTENSITY_THRESHOLD) {
          return; // Ignorar eventos con intensidad baja
        }
    ...
    // Usar un intervalo para verificar y envolver el callback después de que Background.js lo establezca
    // Esto asegura que el callback se envuelva correctamente incluso si Background.js se ejecuta después
    const setupInterval = setInterval(() => {
      if (!onTriggerCallbackRef.current) return;

      // Verificar si ya está envuelto (tiene nuestra propiedad)
    ...
    // Manejar eventos de voz de manera similar
  ...
  }, [onTriggerCallbackRef, onVoiceCallbackRef]);
    ...
    const render = (currentTime) => {
      ...
      activeWaves.forEach(wave => {
        ...
        if (progress >= WAVE_FADE_START) {
          ...
          opacity = BASE_OPACITY * (1 - fadeProgress);
        }
